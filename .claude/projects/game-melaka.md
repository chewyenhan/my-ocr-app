...
- **Worker**：`melaka-ai.chewyenhan.workers.dev`（默认 `gemini-3.5-flash-lite`，前端可选模型，单免费 Key）
...
Worker 端点：`GET /models` + `POST /gemini`。CORS：`chewyenhan.github.io`。限流：15 req/min。API Key：`GEMINI_API_KEY`（Cloudflare Secret，**单免费 Key，无付费兜底**）。模型：前端传参，Worker 透传，默认 `gemini-3.5-flash-lite`。
