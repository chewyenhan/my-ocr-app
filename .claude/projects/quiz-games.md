...
- **Worker**：`hualianhistory-ai.chewyenhan.workers.dev`（`/gemini` 代理 Gemini API，默认 `gemini-3.5-flash-lite`，前端可选模型，单免费 Key）
...
- **已切换**（2026-07-27）：默认 `gemini-3.5-flash-lite`，仅单免费 API Key（移除付费 Key 兜底），Worker 不再硬编码模型——前端 `/models` 探测后可自由切换
